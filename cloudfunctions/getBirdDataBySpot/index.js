...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (spotName) {
      // 如果指定了特定观鸟点
      selectedSpot = SHENZHEN_BIRDING_SPOTS.find(s =>
        s.name === spotName || s.name.includes(spotName) || spotName.includes(s.name)
      );

      if (!selectedSpot) {
        // 尝试模糊匹配
        selectedSpot = SHENZHEN_BIRDING_SPOTS.find(s =>
          spotName.toLowerCase().includes(s.name.substring(0, 2).toLowerCase()) ||
          s.name.toLowerCase().includes(spotName.substring(0, 2).toLowerCase())
        );
      }
      ...
      // 计算经纬度范围（粗略计算）
      const latRange = selectedSpot.radius / 111.0;
      const lngRange = selectedSpot.radius / (111.0 * Math.cos(selectedSpot.coordinates.latitude * Math.PI / 180));

      queryConditions = {
        'location.latitude': _.gte(selectedSpot.coordinates.latitude - latRange)
                              .lte(selectedSpot.coordinates.latitude + latRange),
        'location.longitude': _.gte(selectedSpot.coordinates.longitude - lngRange)
                               .lte(selectedSpot.coordinates.longitude + lngRange),
    ...
    console.error('获取特定观鸟点鸟类数据失败:', error);
    return {
      success: false,
      error: error.message
    };
